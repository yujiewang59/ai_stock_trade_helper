...
import requests
from ai_stock_trade_helper.util.caculate_func import (
...
def StockBasicInfoTool(stock_code: str) -> str:
    indicator_list = [
            '净资产收益率(ROE)',
            '销售净利率',
            '毛利率',
...
# 定义技术面分析工具
def StockTechnicalIndicatorsTool(stock_code: str) -> str:
    # Chinese mapping for indicators
    try:
        price_data: pd.DataFrame = getStockPrice(stock_code)
        ...
        tech_indicators["趋势分析"] = _translate_metrics(trend_signals["metrics"])

        # Calculate mean reversion signals
        mean_reversion_signals = calculate_mean_reversion_signals(price_data)
        tech_indicators["均值回归分析"] = _translate_metrics(mean_reversion_signals["metrics"])

        # Calculate momentum signals
        momentum_signals = calculate_momentum_signals(price_data)
        tech_indicators["动量分析"] = _translate_metrics(momentum_signals["metrics"])

        # Calculate volatility signals
        volatility_signals = calculate_volatility_signals(price_data)
        tech_indicators["波动率分析"] = _translate_metrics(volatility_signals["metrics"])

        # Calculate statistical arbitrage signals
        stat_arb_signals = calculate_stat_arb_signals(price_data)
        tech_indicators["统计套利分析"] = _translate_metrics(stat_arb_signals["metrics"])
...
def _translate_metrics(metrics_dict: Dict[str, Any]) -> Dict[str, Any]:
    """Translate metrics dictionary to Chinese with rounded values"""
    translated = {}

    for key, value in metrics_dict.items():
        chinese_key = INDICATOR_MAPPING.get(key, key)
...
# 定义市场情绪分析工具
def MarketSentimentTool(stock_code: str) -> str:
    try:
        headers = {
...
if __name__ == "__main__":
    # 测试工具函数
    stock_code = "sh600247"
    print(StockBasicInfoTool(stock_code))